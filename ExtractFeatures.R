...
setwd("C:/Users/ADMIN/Desktop/Gender_Identification_by_voice")

data = data.frame("hi-cutiepie.wav", 0, 0, 20)
names(data) <- c('sound.files', 'selec', 'start', 'end')

acoustics <- specan3(data, parallel=1)
write.csv(acoustics, file = "hi-cutiepie.csv")

data = data.frame("myvoice.wav", 0, 0, 20)
names(data) <- c('sound.files', 'selec', 'start', 'end')

acoustics <- specan3(data, parallel=1)
write.csv(acoustics, file = "myvoice.csv")


data = data.frame("myvoice2.wav", 0, 0, 20)
names(data) <- c('sound.files', 'selec', 'start', 'end')

acoustics <- specan3(data, parallel=1)
write.csv(acoustics, file = "myvoice2.csv")
